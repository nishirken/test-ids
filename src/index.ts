...
import {StaticId, StaticIdWithAttrs, DynamicId, DynamicIdWithAttrs, TestId, WithAttrsString} from './types';
...
>(
  ...
  return mapValues(testIds as any, (value: TestId) => { 
    if (isStaticId(value)) {
      ...
      return (attrs: Record<string, any>) => makeSelectorWithAttrs(value, attrs, attrName);
    } else {
...
export function testIdProp<T extends StaticId>(id: T, attrName?: string): Record<string, string>;
export function testIdProp<T extends StaticIdWithAttrs, A extends T['__attrs']>(
  id: T,
  attrs: A,
  attrName?: string
): Record<string, string>;
export function testIdProp(id: string, attrs: any = {}, attrName = 'test-id'): any {
  return mapKeys({
    [attrName]: id.valueOf(),
    ...attrs,
...
export function testIdPropString<T extends StaticId>(id: T, attrName?: string): string;
export function testIdPropString<T extends StaticIdWithAttrs, A extends T['__attrs']>(id: T, attrs: A, attrName?: string): string;
export function testIdPropString(id: string, attrs: any = {}, attrName = 'test-id'): string {
